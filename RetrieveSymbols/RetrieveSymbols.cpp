...
#pragma warning(suppress : 4996)
  ...
  if (symbolPath)
    ...
    printf(
        "_NT_SYMBOL_PATH is not set. Symbol retrieval will probably fail.\n\n");
  ...
  if (_stricmp(ext, ".pdb") == 0) {
    ...
    if (gText.size() != 32) {
      if (gText == gTextArg)
        printf("Error: PDB GUIDs must be exactly 32 characters, length of %s "
               "is %zu.",
               gText.c_str(), gText.size());
      else
        printf("Error: PDB GUIDs must be exactly 32 characters, length of %s "
               "(stripped from %s) is %zu.",
  ...
  if (SymFindFileInPath(fakeProcess, NULL, fileName, id, two, three, flags,
                        filePath, NULL, NULL)) {
